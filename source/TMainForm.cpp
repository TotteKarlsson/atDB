...
#pragma link "dslTLabeledPropertyEdit"
#pragma resource "*.dfm"
...
__fastcall TMainForm::TMainForm(TComponent* Owner)
	...
	mCoverslipPrintingProperties(dsl_shared_ptr<IniFileProperties>(new IniFileProperties)),
    mSplashProperties(dsl_shared_ptr<TRegistryProperties>(new TRegistryProperties(gApplicationRegistryRoot, "Splash"))),
    ...
    mGeneralProperties(dsl_shared_ptr<IniFileProperties>(new IniFileProperties))
{
...
//	    pgDM->connect(mServerIPE->getValue(), mDBUserE->getValue(), mPasswordE->getValue(), mDatabaseE->getValue());
	    pgDM->connect(TPGConnectionFrame1->getCredentials());
    }
}
